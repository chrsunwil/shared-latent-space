...
class dataInfo(dataSetInfoAbstract):
    ...
    def __init__(self):
        ...
        """
        self.name = "Sine"
        self.training_file = os.path.join('Data', 'Sine_Data', 'Training',
                                          'Sine_Training.pkl')
        self.rightXDim = 50
        self.rightYDim = 20
        self.leftXDim = 50
        self.leftYDim = 20
        self.rightDomainName = "Log"
        self.leftDomainName = "Exp2"

        # For Cognoma evaluation, which mutation to induce and compare
        self.mutationID = 485

    def load(self):
        sineData = np.fromfunction(lambda i, j: np.sin(j), (100000, 1000), dtype=float)

        sineData + np.random.uniform(-1, 1, (100000, 1000))

        a_temp = np.exp(sineData)
        x_temp = 2*sineData-5
        ...
        a_temp = a_temp/a_temp.max()
        x_temp = x_temp/x_temp.max()

        where_are_NaNs = np.isnan(x_temp)
        x_temp[where_are_NaNs] = 0

        where_are_NaNs = np.isnan(a_temp)
        a_temp[where_are_NaNs] = 0

        print(a_temp.min())
        print(x_temp.min())

        randIndexes = np.random.randint(0, 100000, (50000,))

        x_temp, a_temp = sk.utils.shuffle(x_temp, a_temp)

        a_temp[:, self.mutationID] = 0
        a_temp[randIndexes, self.mutationID] = 1

        x_temp[:, self.mutationID] = 0
        x_temp[randIndexes, self.mutationID] = 1

        print(np.isnan(a_temp).any())
        print(np.isnan(x_temp).any())
